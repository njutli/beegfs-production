...
**口径B 限速模型（tbf 只限出向 egress；与 JuiceFS+Ceph 完全同口径，直接可比）：**
- **读**（数据 3 slave → 157）：3 个 slave egress 各限 ~118、157 入向不限 → 聚合读天花板 = **3×118 ≈ 354 MiB/s**。故 randread=341、mseqread=332 等 >118 **均可信**，非限速失效。三轮读带宽死平在 340-341，从不越过 354、也远低于 RDMA 级数千，正是"3 个千兆发送端之和"的指纹。
- **写**（数据 157 → slaves）：仅 157 一个发送口 egress 限 ~118 → 聚合写 ≈ **113 MiB/s**。
- ACCEPT=59 = 单链千兆半速；多流读远超 59 属正常达标。
- JuiceFS 侧同样"TBF 只在 3 服务端、客户端不限速、读聚合上限 3×118=354"（`demo/production/prod-deploy/doc/perf-analysis/01-baseline-review-and-nolimit-plan.md §2.2`），两方案口径一致，**无需为对齐而加入向对称限速重测**——加对称入向限速反而模拟不存在的拓扑（生产千兆瓶颈是多客户端各自千兆，正对应此非对称模型）。
- 证据链盲点（不影响结论，下轮补齐）：保存的 IB 采样器测 mlx5（RDMA）计数器，口径B 下全程不变，佐证未走 RDMA，但未直接采 eno12409 字节率。限速生效判据 = beegfs-net 确认 storage 全走 TCP 10.114.1.x + 读340/写113 双向平台与物理预测精确吻合。下轮口径B 按 JuiceFS §9.3 加 eno12409 逐秒 `/proc/net/dev` 直采作冗余证据。

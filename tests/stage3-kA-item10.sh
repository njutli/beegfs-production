#!/bin/bash
set -uo pipefail
# Stage3 口径A item 10: randrw 验收口径 ×3 (fresh dir + create_on_open)
...
log ""
log "=== randrw 验收口径 (fresh dir + create_on_open) ×3 ==="
for i in 1 2 3; do
    rm -rf "${TEST_DIR}"; mkdir -p "${TEST_DIR}"
    drop_all_caches
    mkdir -p "${RESULTS_DIR}/randrw-fresh-r${i}"
    rm -f "${BW_LOG_DIR}/randrw-fresh-r${i}"_bw.*.log 2>/dev/null
    log "## randrw-fresh r${i}: rw=randrw bs=256k 128job create_on_open 180s direct=1"
    fio --directory="${TEST_DIR}" \
        ...
        > "${RESULTS_DIR}/randrw-fresh-r${i}/fio-randrw-fresh-r${i}.txt" 2>&1
    local_rd=$(grep -oP 'READ: bw=\K[0-9.]+' "${RESULTS_DIR}/randrw-fresh-r${i}/fio-randrw-fresh-r${i}.txt" | head -1)
    local_wr=$(grep -oP 'WRITE: bw=\K[0-9.]+' "${RESULTS_DIR}/randrw-fresh-r${i}/fio-randrw-fresh-r${i}.txt" | head -1)
    log "  randrw-fresh r${i}: READ=${local_rd:-NA} WRITE=${local_wr:-NA}"
    cp ${BW_LOG_DIR}/randrw-fresh-r${i}_bw.*.log "${RESULTS_DIR}/randrw-fresh-r${i}/" 2>/dev/null || true
    wait_fio
    sleep 10
done
rm -rf "${TEST_DIR}"

...
> **口径B 限速模型（与 JuiceFS+Ceph 完全同口径，直接可比）**：
> TBF 只加在**发送端 egress**（157 + 3 slave 的 eno12409 出向），tbf 只限出向、不限入向。故：
> - **读**（数据 3 slave → 157）：3 个 slave egress 各限 ~118，157 入向不限 → 聚合读天花板 = **3×118 ≈ 354 MiB/s**，多流读项 >118（如 randread=341）**均可信**，不是限速失效。
> - **写**（数据 157 → slaves）：仅 157 一个发送口 egress 限 ~118 → 聚合写 ≈ **113 MiB/s**。
> - ACCEPT=59 = 单链千兆半速；多流读远超 59 属正常达标，非缓存假象（读带宽三轮死平在 340-341，从不越过 354，也远低于 RDMA 级数千，正是"3 个千兆发送端之和"的指纹）。
> - JuiceFS 侧同样"TBF 只在 3 服务端、客户端不限速、读聚合上限 3×118=354"（见 `demo/production/prod-deploy/doc/perf-analysis/01-baseline-review-and-nolimit-plan.md §2.2`），两方案口径一致，无需为对齐而加入向对称限速重测。
...
> 多流项 >100% 属正常（见本节顶部限速模型：读 = 3 slave 各千兆之和 ≈ 340，写 = 157 单口千兆 ≈ 113）。
>
> **证据链盲点（不影响结论，下轮补齐）**：保存的 IB 采样器测的是 mlx5（100GbE RDMA）计数器，口径B 下 RDMA 计数器全程不变 → 佐证数据没走 RDMA，但**未直接采 eno12409 的字节率**（缺 sar / /proc/net/dev 直采）。限速生效的判据是：beegfs-net 确认 storage 全走 TCP 10.114.1.x + 读340/写113 双向平台与"4 节点 egress 均限速"物理预测精确吻合。下一轮口径B 建议按 JuiceFS §9.3 加 eno12409 逐秒 `/proc/net/dev` 直采作冗余证据。
